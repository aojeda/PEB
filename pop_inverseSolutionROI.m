function EEG = pop_inverseSolutionROI(EEG, windowSize, overlaping, solverType, saveFull, account4artifacts, postprocCallback)
persistent solver
...
for trial=1:EEG.trials
    ...
    for k=1:stepWin:EEG.pnts
        ...
        % Stitch windows
        if k>1 && windowSize > 1
            X(:,loc(1:overlapWin)) = bsxfun(@times, Xtmp(:,1:overlapWin), smoothing) + bsxfun(@times,X(:,loc(1:overlapWin)), 1-smoothing);
            X(:,loc(overlapWin+1:end)) = Xtmp(:,overlapWin+1:end);
        else
            X(:,loc) = Xtmp;
        end
...
EEG.history = char(EEG.history,['EEG = pop_inverseSolutionROI(EEG, ' num2str(windowSize) ', ' num2str(overlaping) ,', ' solverType ', ' num2str(saveFull) ', ' num2str(account4artifacts) ');']);
